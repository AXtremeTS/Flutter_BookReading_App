...
class _BookManagementScreenState extends State<BookManagementScreen> {
  ...
  Future<void> _loadBooks() async {
    final books = await _bookService.adminGetAllBooks();
    if (mounted) {
      setState(() {
        _books = books;
      });
  ...
  Future<void> _deleteBook(String id) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Bạn có chắc chắn muốn xóa cuốn sách này không?'),
        actions: [
          TextButton(
          ...
          TextButton(
            ...
            style: TextButton.styleFrom(foregroundColor: AppColors.accentMagenta),
    ...
    if (confirmed == true) {
      final error = await _bookService.deleteBook(id);
      if (mounted) {
        if (error == null) {
            ...
            child: ListTile(
              ...
              trailing: Row(
                ...
                children: [
                  ...
                  IconButton(
                    icon: Icon(
                      book.isHidden ? Icons.visibility_off : Icons.visibility,
                      color: book.isHidden ? Colors.grey : Colors.blue,
                    ),
                    onPressed: () async {
                      final success = await _bookService.toggleBookVisibility(book.id);
                      if (success) {
                        _loadBooks();
                      }
                    },
                    tooltip: book.isHidden ? 'Hiện sách' : 'Ẩn sách',
                  ),
                  IconButton(
