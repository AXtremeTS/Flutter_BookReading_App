...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadData() async {
    // Bắt đầu trạng thái loading. Không cần `if (mounted)` ở đây
    // vì nó được gọi từ initState, lúc này widget chắc chắn đã mounted.
    setState(() {
      _isLoading = true;
    });

    try {
      // --- DÒNG NÀY ĐỂ KIỂM THỬ ---
  ...
  Future<void> _searchBooks(String query) async {
    final books = await _bookService.getAllBooks(search: query);
    if (mounted) {
      setState(() {
        _displayedBooks = books;
      });
  ...
  Future<void> _filterByTags() async {
    // Note: The backend currently doesn't have a multi-tag filter endpoint, 
    // so we'll fetch all and filter locally for simplicity, 
    // or we can implement it in the backend later.
    final books = await _bookService.getAllBooks();
    if (mounted) {
      setState(() {
        if (_selectedTags.isEmpty) {
          ...
          _displayedBooks = books.where((book) {
            return book.tags.any((tag) => _selectedTags.contains(tag));
          }).toList();
        }
        
        if (_searchController.text.isNotEmpty) {
          final query = _searchController.text.toLowerCase();
          _displayedBooks = _displayedBooks.where((book) {
